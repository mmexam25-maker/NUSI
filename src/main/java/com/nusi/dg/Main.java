...
public final class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        server.createContext("/dg/photo", Main::dgPhoto);
        server.setExecutor(Executors.newVirtualThreadPerTaskExecutor());
        server.start();

        System.out.println("NUSI DG Photo Service started on port " + port);
    }
    ...
    private static void dgPhoto(HttpExchange ex) throws IOException {
        ...
        JsonNode request;
        try {
            request = JSON.readTree(ex.getRequestBody());
        } catch (Exception badJson) {
            sendJson(ex, 400, Map.of("success", false, "message", "Invalid JSON"));
            return;
        }

        String indos = text(request, "indos").toUpperCase().replaceAll("[^A-Z0-9]", "");
        String password = text(request, "password");

        if (!INDOS.matcher(indos).matches()) {
            sendJson(ex, 400, Map.of(
                    "success", false,
                    "message", "Invalid INDoS format. Expected 00AA0000."
            ));
            return;
        }

        if (password.isBlank()) {
            sendJson(ex, 400, Map.of("success", false, "message", "Password is required."));
            return;
        }
        ...
        try {
            ...
            if (!acquired) {
                sendJson(ex, 429, Map.of(
                        "success", false,
                        "message", "DG photo service is busy. Please retry in a few seconds."
                ));
                return;
            }

            System.out.println("DG PHOTO REQUEST | INDoS " + indos);
            ...
            System.out.println("DG PHOTO SUCCESS | INDoS " + indos + " | " + result.width() + "x" + result.height());
    ...
    private static String safeMessage(Throwable e) {
        ...
        if (m == null || m.isBlank()) return "DG photo retrieval failed.";
        // Never echo a secret if some lower-level exception contains it.
