...
public final class Main {
    ...
    private static void submit(HttpExchange ex) throws IOException {
        ...
        try {
            ...
            byte[] cdc = decodeFile(data.get("cdcfile"), "CDC", "application/pdf");
            byte[] passport = decodeFile(data.get("passport"), "Passport", "application/pdf");
            ...
            GoogleStore.SavedMember saved = GoogleStore.saveMember(data, photoBytes, cdc, passport);
    ...
    private static void serveResource(HttpExchange ex, String resource, String contentType, boolean cache) throws IOException {
        ...
        // Render runtime: serve directly from /app/public first.
        String fileName = resource.replace("/public/", "");
        java.nio.file.Path diskFile = java.nio.file.Path.of("/app/public", fileName);

        if (java.nio.file.Files.exists(diskFile) && java.nio.file.Files.isRegularFile(diskFile)) {
            bytes = java.nio.file.Files.readAllBytes(diskFile);
        }

        // Local/dev fallback: serve from the JAR classpath.
        if (bytes == null) {
